...
| `GET` | `/admin/all_orders?limit=20&offset=0&created_at_from=2026-06-16T00:00:00%2B05:00&created_at_to=2026-06-17T00:00:00%2B05:00&status=packed` | admin JWT | получить все заказы с необязательными фильтрами |
...
- `limit`: от `1` до `500`, по умолчанию `20`
- `offset`: от `0`, по умолчанию `0`
- `created_at_from`: необязательное начало промежутка, datetime должен содержать timezone offset
- `created_at_to`: необязательный конец промежутка, datetime должен содержать timezone offset
- `status`: необязательная строка со статусом заказа

Backend переводит `created_at_from` и `created_at_to` в UTC перед фильтрацией. Если пишете raw URL, кодируйте `+` в offset как `%2B`. Если промежуток datetime и `status` не переданы, `/admin/all_orders` возвращает все заказы от новых к старым. Элементы ответа содержат обычные поля заказа и `user_id`.

Если `created_at_from` больше `created_at_to`, backend возвращает `400 Bad Request`.
...
| `400 Bad Request` | некорректный datetime-промежуток в админском списке всех заказов |
